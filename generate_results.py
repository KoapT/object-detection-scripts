...
if __name__ == '__main__':
    modelDir = '/home/tk/Desktop/machine_learning/tensorFlowTrain/models_custom/object_detection/tfyolov3/saved_model_pb/'
    ...
    with open(valid_file) as f:
        valid_path = f.readlines()
    images_path = []
    for i in range(0, len(valid_path), 8):
        images_path = valid_path[i:i + 8]
        images_list = []
        for img_path in images_path:
            img_path = img_path.strip()
            images_list.append(cv.imread(img_path))
        rst_images = net.callNet(images_list)
        for n, boxes in enumerate(rst_images):
            filename = images_path[n].strip().split('/')[-1][:-4]
            for box in boxes:
                box.append(filename)
                if box[4] == 1:
                    bird.append(box)
    ...
    with open('comp4_bird.txt', 'w') as b:
        for box in bird:
            b.write(box[6] + ' ' + str(box[5]) + ' ' + ' '.join([str(i) for i in box[:4]]) + '\n')

    with open('comp4_person.txt', 'w') as p:
        for box in person:
            b.write(box[6] + ' ' + str(box[5]) + ' ' + ' '.join([str(i) for i in box[:4]]) + '\n')
        # for i in range(10):
        #     rst_images = net.callNet(images_list)
